...
public:
    Field re, im;
    static constexpr uint32_t NON_RESIDUE = 3;
...
namespace sumcheck {
template <> struct FieldTraits<koalabear::Ext2> {
    static void absorb(Transcript& ts, const koalabear::Ext2& e) { ts.absorb_field_element(e.re); ts.absorb_field_element(e.im); }
    static koalabear::Ext2 squeeze(Transcript& ts) { return {ts.squeeze_challenge_as<koalabear::Field>(), ts.squeeze_challenge_as<koalabear::Field>()}; }
};
template <> struct FieldTraits<koalabear::Ext4> {
    static void absorb(Transcript& ts, const koalabear::Ext4& e) { ts.absorb_field_element(e.low); ts.absorb_field_element(e.high); }
    static koalabear::Ext4 squeeze(Transcript& ts) { return {ts.squeeze_challenge_as<koalabear::Ext2>(), ts.squeeze_challenge_as<koalabear::Ext2>()}; }
};
}